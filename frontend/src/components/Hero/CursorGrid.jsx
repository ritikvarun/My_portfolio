...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    const isInsideOrNear = (e) => {
      const rect = canvas.getBoundingClientRect();
      const r = propsRef.current.radius || 140;
      return (
    ...
    const triggerAutoFlash = () => {
      const p = propsRef.current;
      if (!p.autoFlash || w === 0 || h === 0) return;
      if (typeof document !== "undefined" && document.hidden) return;

      // Random point across the grid area
      const randX = Math.random() * (w - 40) + 20;
      const randY = Math.random() * (h - 40) + 20;

      // Energize surrounding cells with random intensity
      energize(randX, randY, Math.random() * 0.4 + 0.6);

      // Trigger an expanding wave pulse
      if (p.clickPulse) {
        pulses.push({ x: randX, y: randY, t0: performance.now() });
      }
    ...
    const scheduleNextFlash = () => {
      const p = propsRef.current;
      if (!p.autoFlash) return;
      const interval = p.flashInterval || 2000;
      // Add natural random organic variation (+- 30%)
      const jitter = (Math.random() - 0.5) * (interval * 0.5);
      const nextTime = Math.max(700, interval + jitter);
    ...
    // Trigger initial flash shortly after mount/rebuild and start loop
    if (propsRef.current.autoFlash) {
      flashTimeout = setTimeout(() => {
        triggerAutoFlash();
        scheduleNextFlash();
      }, 500);
